...
contract PokerGame {
  ...
  uint256 public betAmount = uint256(1 ether).div(10);
  IERC20 public betToken;
  string[13] denominations = [
    "A",
    "2",
    "3",
    "4",
  ...
  function setAmount(uint256 _amount) public payable {
    playerCurrentAmount[msg.sender] = playerCurrentAmount[msg.sender].add(
      _amount
    );
    dealerCurrentAmount[msg.sender] = dealerCurrentAmount[msg.sender].add(
      _amount
    );
  }
  ...
  function placeBet(uint256 _amount) public {
    ...
    dealerCurrentAmount[msg.sender] = dealerCurrentAmount[msg.sender].sub(
      _amount
    );
  ...
  function rewardWinner(string memory _winner) public {
    ...
    if (
      ...
      playerCurrentAmount[msg.sender] = playerCurrentAmount[msg.sender]
        .add(bidAmount[msg.sender].mul(2));
    }
    if (
      (keccak256(abi.encodePacked(_winner)) ==
        keccak256(abi.encodePacked("dealer")))
    ) {
      dealerCurrentAmount[msg.sender] = dealerCurrentAmount[msg.sender]
        .add(bidAmount[msg.sender].mul(2));
    }
    if (
      (keccak256(abi.encodePacked(_winner)) ==
        keccak256(abi.encodePacked("tie")))
    ) {
      dealerCurrentAmount[msg.sender] = dealerCurrentAmount[msg.sender]
        .add(bidAmount[msg.sender]);
      playerCurrentAmount[msg.sender] = playerCurrentAmount[msg.sender]
        .add(bidAmount[msg.sender]);
    }

    bidAmount[msg.sender] = bidAmount[msg.sender].sub(
      bidAmount[msg.sender]
    );
  }
  ...
  function setHand(string memory _playerHand, string memory _dealerHand)
    public
  {
